...
class Bank(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.branch_digit = smarter_zfill(
            kwargs.get('branch_digit', ''), self.branch_digit_length
        )
        self.account = smarter_zfill(kwargs['account'], self.account_length)
        self.account_digit = smarter_zfill(
            kwargs.get('account_digit', ''), self.account_digit_length
        )
    ...
    def get(cls, bank_code, country=None):
        ...
        bank_class = list(
            filter(
                lambda x: x.bank_code == bank_code and x.country == country,
                subclasses,
            )
        )
    ...
    def execute(self):
        ...
        if not self.validate():
            raise InvalidBranchAndAccountCombination(
                self.branch,
                self.branch_digit,
                self.account,
                self.account_digit,
...
class Santander(BrazilianBank):
    ...
    def validate(self):
        ...
        dv = calculate_verifier_digit(
            account_relevant_data, pivot='97310097131973', method='mod10'
        )
...
class Banrisul(BrazilianBank):
    ...
    def validate_branch_digit(self):
        ...
        first_digit = 10 - (
            (
                sum_digits(int(self.branch[0]) * 1)
                + sum_digits(int(self.branch[1]) * 2)
                + sum_digits(int(self.branch[2]) * 1)
                + sum_digits(int(self.branch[3]) * 2)
            )
            % 10
        )
        ...
        return (
            self.branch_digit.lower()
            == '{}{}'.format(first_digit, second_digit).lower()
        )
